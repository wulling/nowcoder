...
public class LoginController implements CommunityConstant {
    ...
    private String registere(Model model,User user){  //只要页面上传入的值与user的属性相匹配，spring mvc会自动把这个值注入给user对象的属性
...
//        session.setAttribute("kaptcha",text);
    ...
    public String login(String username,String password,String code,boolean rememberme,
        ...
        //如果这个参数是实体类，比如user,那么spring mvc会将这个user装到model中，但是参数是普通的像字符串，基本类型这样的，spring mvc就不会将参数放入model
        //检查验证码
//        String kaptcha = (String) session.getAttribute("kaptcha");
    ...
    public String getcode(String email, Model model, HttpSession session){
        ...
        if(map.containsKey("code")){
            ...
            return CommunityUtil.getJSONString(1,"验证码发送失败");
        }
    }
